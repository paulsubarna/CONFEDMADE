...
x_temp = X_train #np.concatenate((X_train, X_test))
...
labels_per_task= [[5, 8], [1, 4], [3, 6],
                  ...
                  [3, 7], [2, 6], [4, 8],
                  [4, 9], [3, 8], [0, 9]]
base_dir = '/app/src/FedWit/content'
m_d= False
if m_d:
...
for i in range(len(data_split)):
  ...
  if data_split[i] == 'full':
    for i,labels in enumerate(labels_per_task):
      ...
      idx = np.concatenate([np.where(y[:]==l)[0] for l in labels], axis=0)
  ...
  for _seed in seeds:
    ...
    for t in range(num_tasks):
      for r in range(num_rounds):
        client_weights= []
        for c in range(num_clients):
          print(f"Task {t} at round {r} for Client {c}")
