...
random_shuffle(seed, idx_shuffled)
x = x[idx_shuffled]
y = y[idx_shuffled]
X_valid= x[60000:]
num_clients= 3
...
labels_per_task= [[5, 8], [1, 4], [3, 6],
                  ...
                  [3, 7], [2, 6], [4, 8],
                  [4, 9], [3, 8], [0, 9]]
base_dir = '/app/src/FedWit/output/data'
for i,labels in enumerate(labels_per_task):
  ...
  val= data[train_len:]
  filename= f'Task_{i}'
  np_save(base_dir, filename, train)
  v_f = f'Val_{i}'
...
for i in range(len(data_split)):
  ...
  #random_shuffle(seed+ 100, idx_shuffled)
  ...
  elif data_split[i] == 'voll':
    for c in range(num_clients):
      idx_shuffled = np.arange(len(X_train))
      random_shuffle(seed+ 70, idx_shuffled)
      X_train = X_train[idx_shuffled]
      data= X_train[:6000]
      filename= f'Task_{c}'
      np_save(base_dir, filename, data)
  ...
  for _seed in seeds:
    ...
    for t in range(num_tasks):
      for r in range(num_rounds):
        client_weights= []
        for c in range(num_clients):
          X= np.load(f'/app/src/FedWit/output/data/Task_{t*num_clients+ c}.npy')
          history = model.fit(
              X, X,
          ...
          if t > 0:
            for pt in range(t):
              val= np.load(f'/app/src/FedWit/output/data/Task_{(pt)*num_clients+ c}.npy')
              test_loss=model.evaluate(val, val, batch_size=_batch_size)
              f = open("/app/src/FedWit/output/loss_clients.txt", "a")
