...
def cross_entropy_loss(x, x_decoded_mean):
    ...
    xent_loss = 500 * metrics.binary_crossentropy(x, x_decoded_mean)
  ...
  for _seed in seeds:
    ...
    for t in range(num_tasks):
      for r in range(num_rounds):   
        client_weights= []
        for c in range(num_clients):
          data= np.load(f'/app/src/FedWit/output/non_miid/NON_IID_{t*num_clients+ c}_train.npy', allow_pickle=True).item()
          ...
          if t > 0:
            for pt in range(t):
              val_data= np.load(f'/app/src/FedWit/output/non_miid/NON_IID_{(pt)*num_clients+ c}_valid.npy', allow_pickle=True).item()
              val= val_data['x_valid']
              test_loss=model.evaluate(val, val, batch_size=_batch_size)
              f = open("/app/src/FedWit/loss_clients_2.txt", "a")
