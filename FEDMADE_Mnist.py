  ...
  for _seed in seeds:
    ...
    for t in range(num_tasks):
      for r in range(num_rounds):   
        client_weights= []
        for c in range(num_clients):
          data= np.load(f'/app/src/FedWit/output/mnist/mnist_{t*num_clients+ c}_train.npy', allow_pickle=True).item()
          X=  tf.convert_to_tensor(data['x_train'])
          val_data= np.load(f'/app/src/FedWit/output/mnist/mnist_{t*num_clients+ c}_valid.npy', allow_pickle=True).item()
          ...
          if t > 0:
            for pt in range(t):
              val_data= np.load(f'/app/src/FedWit/output/mnist/mnist_{(pt)*num_clients+ c}_test.npy', allow_pickle=True).item()
              val= val_data['x_test']
              test_loss=model.evaluate(val, val, batch_size=_batch_size)
              f = open("/app/src/FedWit/incremental_ub_femade.txt", "a")
