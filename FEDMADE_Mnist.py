...
class DirectInputConnectConditionningMaskedLayer(ConditionningMaskedLayer):
      ...
      def call(self, inputs):
        ...
        weighted_input_and_bias = k.bias_add(weighted_input, self.bias, data_format = 'channels_last')
        #print(weighted_input_and_bias.shape)
        weighted_direct_input = k.dot(direct_input, self.D * self.direct_mask)
        #print(weighted_direct_input.shape)
        if self.use_cond_mask:
  ...
  def build_model(self):
    ...
    for i in range(1,len(self.units_per_layer)-1): #exclude input & output layer
      if i == 1:
        ...
        x_layers.append(ConditionningMaskedLayer(units = self.units_per_layer[i], mask = masks[i-1], use_cond_mask = self.connectivity_weights)(x_layers[i-2]))
...
for i in range(len(data_split)):
  ...
  if data_split[i] == 'full':
    for i,labels in enumerate(labels_per_task):

      idx = np.concatenate([np.where(y[:]==l) for l in labels], axis=0)
  ...
  for _seed in seeds:
    ...
    for t in range(num_tasks):
      for r in range(num_rounds):   
        client_weights= []
        for c in range(num_clients):
          data= np.load(f'output/mnist/mnist_{t*num_clients+ c}_train.npy', allow_pickle=True).item()
          X=  tf.convert_to_tensor(data['x_train'])
          val_data= np.load(f'output/mnist/mnist_{t*num_clients+ c}_valid.npy', allow_pickle=True).item()
          ...
          if t > 0:
            for pt in range(t):
              val_data= np.load(f'output/mnist/mnist_{(pt)*num_clients+ c}_test.npy', allow_pickle=True).item()
              val= val_data['x_test']
              test_loss=model.evaluate(val, val, batch_size=_batch_size)
              f = open("results/mnist_ablation2.txt", "a")
        ...
        model.set_weights(el)

    done = time.time()
    elapsed = done - start
    print("Elapsed: ", elapsed)
    print(f"Number of masks: {_num_masks}")
    X_test= np.load(f'output/mnist/mnist_full_test_set.npy', allow_pickle=True).item()
