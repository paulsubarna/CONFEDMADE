...
for i in range(len(data_split)):
  ...
  for _seed in seeds:
    ...
    for t in range(num_tasks):
      ...
      data= np.load(f'/app/src/FedWit/output/non_miid/NON_IID_{t}_train.npy', allow_pickle=True).item()
      X=  tf.convert_to_tensor(data['x_train'])
      data_val= np.load(f'/app/src/FedWit/output/non_miid/NON_IID_{t}_valid.npy', allow_pickle=True).item()
      ...
      if t > 0:
        for pt in range(t):
          val_data= np.load(f'/app/src/FedWit/output/non_miid/NON_IID_{pt}_valid.npy', allow_pickle=True).item()
          val= val_data['x_valid']
          test_loss=model.evaluate(val, val, batch_size=_batch_size)
          f = open("/home/subarna/Pictures/LARS/FedWeIT-MADE/output/loss_clients_2.txt", "a")
      ...
      f = open("/app/src/FedWit/output/loss_clients_made.txt", "a")
      f.write('\n'+
              f'[task {t}]'+
              f'[round {r}]'+
              f'[client {c}] '+
