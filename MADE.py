...
def cross_entropy_loss(x, x_decoded_mean):
    ...
    xent_loss = 190 * metrics.binary_crossentropy(x, x_decoded_mean)
...
units_per_layer = np.concatenate(([190], _hidden_layers, [190])) #in MADE case the input & output layer have the same amount of units
print("shape",units_per_layer)
seeds= [777]
#for i in range(1):
#  seeds.append(np.random.randint(800))
print(seeds)
data_split= ['sull']#['full', 12000, 6000, 3000, 1500]
clients= [5]#[3,5,10,20,40]
for i in range(len(data_split)):
  ...
  for _seed in seeds:
    ...
    for t in range(num_tasks):
      ...
      data= np.load(f'/home/subarna/Pictures/LARS/FedWeIT-MADE/output/binary/binary_{t}_train.npy', allow_pickle=True).item()
      X=  tf.convert_to_tensor(data['x_train'])
      #data_val= np.load(f'/home/subarna/Pictures/LARS/FedWeIT-MADE/output/binary/binary_{t}_valid.npy', allow_pickle=True).item()
      ...
      #temp_val = history.history['val_loss']


      if t > 0:
        for pt in range(t):
          val_data= np.load(f'/home/subarna/Pictures/LARS/FedWeIT-MADE/output/binary/binary_{pt}_valid.npy', allow_pickle=True).item()
          val= val_data['x_valid']
          test_loss=model.evaluate(val, val, batch_size=_batch_size)
          f = open("/home/subarna/Pictures/LARS/FedWeIT-MADE/output/loss_clients_binary.txt", "a")
          f.write('\n'+f'forgetting loss of task  {pt}  at current task {t}: {test_loss}' )
          f.close()
...
model.save('/app/src/FedWit/mymodel')
